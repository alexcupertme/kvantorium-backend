...
describe("POST /user/getmyinfo", () => {
	...
	describe("1. if token didnt sent", () => {
		beforeAll(async () => {
			res = await request(app.app).post(`/api/user/getmyinfo`).set("Content-Type", "application/json").set("Accept", "application/json");
		});
	...
	describe("2. if token is correct, it should", () => {
		beforeAll(async () => {
			const userData = {
				login: "unknownuser",
				description: "Привет, мир!",
			};
			res = await request(app.app).post(`/api/user/getmyinfo`).send(userData).set("Content-Type", "application/json").set("Accept", "application/json").set("Cookie", [cookie]);
		});
