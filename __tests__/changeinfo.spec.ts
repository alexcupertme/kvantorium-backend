...
describe("POST /user/changeinfo", () => {
	...
	describe("1. if token didnt sent", () => {
		beforeAll(async () => {
			...
			res = await request(app.app).post(`/api/user/changeinfo`).send(userData).set("Content-Type", "application/json").set("Accept", "application/json");
		});
	...
	describe("2. if data isnt correct", () => {
		beforeAll(async () => {
			const userData = {
				login:
	...
	describe("3. if mail already registered", () => {
		beforeAll(async () => {
			const userData = {
				mail: "teledurak@gmail.com",
			};
	...
	describe("4. if login already in use", () => {
		beforeAll(async () => {
			const userData = {
				login: "test",
			};
